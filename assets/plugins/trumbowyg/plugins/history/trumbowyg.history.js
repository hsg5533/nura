      ...
      history: {
        init: function (t) {
          ...
          var btnBuildDefRedo = {
            ...
            fn: function () {
              if (
                ...
                t.o.plugins.history._stack.length - 1
              ) {
          ...
          var pushToHistory = function () {
            ...
            latestStateTagsList = $("<div>" + latestState + "</div>")
              .find("*")
              .map(function () {
                return this.localName;
              });
            newStateTagsList = $("<div>" + newState + "</div>")
              .find("*")
              .map(function () {
                return this.localName;
              });
            ...
            if (newState !== prevState) {
              ...
              if (
                focusElText.slice(-1).match(/\s/) ||
                !arraysAreIdentical(latestStateTagsList, newStateTagsList) ||
                t.o.plugins.history._index <= 0 ||
                focusEl !== prevFocusEl
              ) {
                t.o.plugins.history._index += 1;
