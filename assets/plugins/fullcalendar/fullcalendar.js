...
----------------------------------------------------------------------------------------------------------------------*/
  ...
  function isTimeString(str) {
    return (
      typeof str === "string" && /^\d+\:\d+(?:\:\d+\.?(?:\d{3})?)?$/.test(str)
    );
  }
...
*/
  ...
  var EventDragging = (FC.EventDragging = Interaction.extend({
	...
	*/
    ...
    computeEventDropMutation: function (
      startFootprint,
      endFootprint,
      eventDef
    ) {
  ...
  var EventResizing = (FC.EventResizing = Interaction.extend({
	...
	*/
    ...
    computeEventStartResizeMutation: function (
      startFootprint,
      endFootprint,
      origEventFootprint
    ) {
    ...
    computeEventEndResizeMutation: function (
      startFootprint,
      endFootprint,
      origEventFootprint
    ) {
  ...
  var ExternalDropping = (FC.ExternalDropping = Interaction.extend(
    ListenerMixin,
    {
      dragListener: null,
	...
	*/
      ...
      listenToExternalDrag: function (el, ev, ui) {
        ...
        var dragListener = (_this.dragListener = new HitDragListener(
          component,
          {
            interactionStart: function () {
...
----------------------------------------------------------------------------------------------------------------------*/
  ...
  var HelperRenderer = (FC.HelperRenderer = Class.extend({
    ...
    renderEventDraggingFootprints: function (
      eventFootprints,
      sourceSeg,
      isTouch
    ) {
    ...
    renderEventResizingFootprints: function (
      eventFootprints,
      sourceSeg,
      isTouch
    ) {
    ...
    renderEventFootprints: function (
      eventFootprints,
      sourceSeg,
      extraClassNames,
      opacity
  ...
  var InteractiveDateComponent = (FC.InteractiveDateComponent =
    DateComponent.extend({
      ...
      canStartResize: function (seg, ev) {
        ...
        return (
          (!getEvIsTouch(ev) || view.isEventDefSelected(eventDef)) &&
      ...
      isExternalInstanceGroupAllowed: function (eventInstanceGroup) {
        ...
        for (i = 0; i < eventFootprints.length; i++) {
          ...
          if (
            !view.calendar.isSelectionFootprintAllowed(
...
----------------------------------------------------------------------------------------------------------------------*/
  ...
  var View = (FC.View = InteractiveDateComponent.extend({
    ...
    bindGlobalHandlers: function () {
      InteractiveDateComponent.prototype.bindGlobalHandlers.apply(
        this,
        arguments
      );
    ...
    unbindGlobalHandlers: function () {
      InteractiveDateComponent.prototype.unbindGlobalHandlers.apply(
        this,
        arguments
      );
    ...
    renderSelectionFootprint: function (footprint, ev) {
      if (this.renderSelection) {
        ...
        InteractiveDateComponent.prototype.renderSelectionFootprint.apply(
          this,
          arguments
        );
      }
  ...
  View.mixin({
    ...
    buildPrevDateProfile: function (date) {
      ...
      var prevDate = date
        .clone()
        .startOf(dateProfile.currentRangeUnit)
    ...
    function activateButton(buttonName) {
      if (el) {
        el.find(".fc-" + buttonName + "-button").addClass(
          calendar.theme.getClass("stateActive")
        );
      }
    }
  ...
  Calendar.mixin({
    ...
    initialRender: function () {
      ...
      this.optionsModel.watch(
        ...
        ["?theme", "?themeSystem"],
        function (opts) {
          var themeClass = ThemeRegistry.getThemeClass(
      ...
      this.optionsModel.watch(
        ...
        ["?businessHours"],
        function (deps) {
          _this.businessHourGenerator = new BusinessHourGenerator(
            deps.businessHours,
            _this
          );
...
*/
  Calendar.prototype.isEventInstanceGroupAllowed = function (
    eventInstanceGroup
  ) {
  ...
  Calendar.prototype.isSelectionFootprintAllowed = function (
    componentFootprint
  ) {
  ...
  Calendar.prototype.isFootprintWithinConstraints = function (
    componentFootprint,
    constraintFootprints
  ) {
    var i;
  ...
  Calendar.prototype.constraintValToFootprints = function (
    constraintVal,
    isAllDay
  ) {
    var eventInstances;
  ...
  Calendar.prototype.collectOverlapEventFootprints = function (
    peerEventFootprints,
    targetFootprint
  ) {
  ...
  function isOverlapEventInstancesAllowed(
    ...
    for (i = 0; i < overlapEventFootprints.length; i++) {
      ...
      if (overlapVal === false) {
        ...
        if (
          !overlapVal(overlapEventInstance.toLegacy(), subjectLegacyInstance)
        ) {
          return false;
        }
      }
...
*/
  ...
  Calendar.prototype.footprintContainsFootprint = function (
    outerFootprint,
    innerFootprint
  ) {
  ...
  var UnzonedRange = (FC.UnzonedRange = Class.extend({
    ...
    equals: function (otherRange) {
      return (
        this.startMs === otherRange.startMs && this.endMs === otherRange.endMs
      );
    },
...
*/
  ...
  var EventDefParser = {
    parse: function (eventInput, source) {
      if (
        isTimeString(eventInput.start) ||
        moment.isDuration(eventInput.start) ||
        isTimeString(eventInput.end) ||
  ...
  var SingleEventDef = EventDef.extend({
	...
	*/
    applyManualStandardProps: function (rawProps) {
      var superSuccess = EventDef.prototype.applyManualStandardProps.apply(
        this,
        arguments
      );
  ...
  var EventInstanceGroup = (FC.EventInstanceGroup = Class.extend({
    ...
    sliceNormalRenderRanges: function (constraintRange) {
      ...
      for (i = 0; i < eventInstances.length; i++) {
        ...
        slicedRange =
          eventInstance.dateProfile.unzonedRange.intersect(constraintRange);

        if (slicedRange) {
          slicedEventRanges.push(
            new EventRange(slicedRange, eventInstance.def, eventInstance)
          );
        }
      }
...
*/
  ...
  var ArrayEventSource = EventSource.extend({
	...
	*/
    ...
    applyManualStandardProps: function (rawProps) {
      var superSuccess = EventSource.prototype.applyManualStandardProps.apply(
        this,
        arguments
      );
...
----------------------------------------------------------------------------------------------------------------------*/
  ...
  var DayGrid = (FC.DayGrid = InteractiveDateComponent.extend(
	...
	------------------------------------------------------------------------------------------------------------------*/
      ...
      unrenderEvents: function () {
        ...
        InteractiveDateComponent.prototype.unrenderEvents.apply(
          this,
          arguments
        );
      },
      ...
      getOwnEventSegs: function () {
        return InteractiveDateComponent.prototype.getOwnEventSegs
          .apply(this, arguments) // get the segments from the super-method
  ...
  DayGrid.mixin({
    ...
    resliceDaySegs: function (segs, dayDate) {
      ...
      for (i = 0; i < segs.length; i++) {
        seg = segs[i];
        slicedRange =
          seg.footprint.componentFootprint.unzonedRange.intersect(dayRange);
...
----------------------------------------------------------------------------------------------------------------------*/
  ...
  var BasicView = (FC.BasicView = View.extend({
    ...
    buildRenderRange: function (
      ...
      var renderUnzonedRange = View.prototype.buildRenderRange.apply(
        this,
        arguments
      ); // an UnzonedRange
      var start = this.calendar.msToUtcMoment(
        renderUnzonedRange.startMs,
        isRangeAllDay
      );
      var end = this.calendar.msToUtcMoment(
  ...
  var basicDayGridMethods = {
    ...
    getIsNumbersVisible: function () {
      return (
        DayGrid.prototype.getIsNumbersVisible.apply(this, arguments) ||
...
----------------------------------------------------------------------------------------------------------------------*/
  ...
  var MonthView = (FC.MonthView = BasicView.extend({
    ...
    buildRenderRange: function (
      ...
      var renderUnzonedRange = BasicView.prototype.buildRenderRange.apply(
        this,
        arguments
      );
      var start = this.calendar.msToUtcMoment(
...
----------------------------------------------------------------------------------------------------------------------*/
  ...
  var TimeGrid = (FC.TimeGrid = InteractiveDateComponent.extend(
    StandardInteractionsMixin,
    DayTableMixin,
    {
      ...
      renderSlatRowHtml: function () {
        ...
        while (slotTime < dateProfile.maxTime) {
          slotDate = calendar
            ...
            .time(slotTime);
          isLabeled = isInt(
            divideDurationByDuration(slotIterator, this.labelInterval)
          );
  ...
  var ListView = (FC.ListView = View.extend({
    ...
    renderDates: function (dateProfile) {
      ...
      while (dayStart < viewEnd) {
        ...
        dayRanges.push(
          new UnzonedRange(dayStart, dayStart.clone().add(1, "day"))
        );
